...
- A **LoadBalancer implementation** such as [MetalLB](https://metallb.universe.tf/) — required to assign an IP to `vtom-server` and `mft-sftp` Services. Without it, `type: LoadBalancer` Services remain in `<pending>` state indefinitely. On bare-metal or local clusters (RKE2, kubeadm, Minikube), this is not provided by default
...
| On-premise | none — requires a LoadBalancer implementation (e.g. [MetalLB](https://metallb.universe.tf/)) |
